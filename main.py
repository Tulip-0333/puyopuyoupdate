...
class Game(object):
    ...
    def __init__(self):
        ...
        self.gem = {
            "R": pygame.image.load("resource/r.png").convert_alpha(),
    ...
    def draw(self, p):
        ...
        for y, row in enumerate(p.puyos):
            for x, color in enumerate(row):
                if color != " ":
                    self.screen.blit(
                        self.gem[color], (x_offset + x * 24, y_offset + y * 24)
                    )
        if p.falling:
            for i in range(2):
                y, x = p.falling[i]["pos"]
                if y >= 0 and x >= 0:
                    self.screen.blit(
                        self.gem[p.falling[i]["color"]],
                        (x_offset + x * 24, y_offset + y * 24),
                    )
        ...
        while True:
            ...
            if self.player1.falling and not (counter % 3):
                ...
                if keys[self.player1.controller["left"]]:
                    if (row1 > 0 and self.player1.puyos[col1][row1 - 1] == " ") and (
                        row2 > 0 and self.player1.puyos[col2][row2 - 1] == " "
                    ):
                        self.player1.falling[0]["pos"] = (col1, row1 - 1)
                        self.player1.falling[1]["pos"] = (col2, row2 - 1)
                if keys[self.player1.controller["right"]]:
                    if (
                        row1 < self.player1.WIDTH - 1
                        and self.player1.puyos[col1][row1 + 1] == " "
                    ) and (
                        row2 < self.player1.WIDTH - 1
                        and self.player1.puyos[col2][row2 + 1] == " "
                    ):
                        self.player1.falling[0]["pos"] = (col1, row1 + 1)
                        self.player1.falling[1]["pos"] = (col2, row2 + 1)
                if keys[self.player1.controller["down"]]:
                    if (
                        col1 < self.player1.HEIGHT - 1
                        and self.player1.puyos[col1 + 1][row1] == " "
                    ) and (
                        col2 < self.player1.HEIGHT - 1
                        and self.player1.puyos[col2 + 1][row2] == " "
                    ):
                ...
                if keys[self.player1.controller["roll"]]:
                    ...
                    a2 = row1 - row2
                    if (
                        (row1 + a1 in (-1, self.player1.WIDTH))
                        or col1 - a2 == self.player1.HEIGHT
                        or self.player1.puyos[col1 - a2][row1 + a1] != " "
                    ):
                        pass
                    else:
